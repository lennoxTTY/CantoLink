...
import pysrt, auditok, os, ffmpeg, requests, uuid, math, datetime, shutil,threading,timeit
...
key = "99fe8e015c004b6fa3d86acd7473772f"
speech_key= "c4aaa98fa2c243269fc642b91f294ef6"
location = "eastasia"
endpoint = 'https://api.cognitive.microsofttranslator.com'
params ='&from=yue&to=zh-Hant'
...
def translator(index,original_text,list):
    ...
    translate_req = requests.post(constructed_url, headers=headers, json=body)
    transText=translate_req.json()[0]['translations'][0]['text']
    list[index] = transText
...
def takeFile():
    ...
    if request.method=='POST':
        ...
        previewGenThread = threading.Thread(target=previewGen, args=[filename])
        previewGenThread.start()
        ...
        return render_template('processed.html',len=len(Outputfile),subtitles=Outputfile,time=f"處理時間：{totalTimeE-totalTimeS}")
    else:
        no_file_error("無法讀取檔案")
